...
import { formLoginSchema, type FormLoginData } from './-features/auth-schema';
import { PostAuth, saveTokens } from './-features/auth-service';
...
function LoginComponent() {
  ...
  async function onSubmit(data: FormLoginData) {
    try {
      const response = await PostAuth("Account/login", data);
      ...
      form.setError("root", {
        message: errorMessage[message ?? ""] ?? "Erro ao registrar usuário"
      });
    };
  };
    ...
    <div>
      <form className='w-[300px] mt-6' onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <Button
          ...
          className='w-full hover:cursor-pointer rounded-sm transition-colors bg-[#34397B] hover:bg-[#2c2863]' 
        >
      ...
      <p className='text-center text-sm text-zinc-400 mt-6'>
        ...
        <Link to='/register' className='text-sm underline transition-colors text-[#94ADFF]  hover:text-white/80'>Criar agora</Link>
      </p>
    </div>
  )
}
